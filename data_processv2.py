...
from transformers import CLIPProcessor, CLIPModel,CLIPTextModel,CLIPTokenizer,CLIPTokenizerFast,AutoProcessor
# debug
# flagflag=0
...
class SentenceDataset(Dataset):
    def __init__(self, opt, data_path, text_tokenizer, photo_path, image_transforms, data_type, data_translation_path=None, image_coordinate=None):
        ...
        if opt.text_model == 'bert-base' or opt.text_model == 'bertweet-en' or opt.text_model == 'debertav3_base':
        ...
        clip_tokenizer=CLIPTokenizerFast.from_pretrained('./clip-vit-base-patch32')
        clip_input=clip_tokenizer(self.text_list,truncation=True,max_length=50,padding='max_length', return_tensors="pt")

        self.clip_to_id=clip_input['input_ids']
        self.clip_attention_mask=clip_input['attention_mask']

        self.processor = AutoProcessor.from_pretrained('./clip-vit-base-patch32')
    ...
    def __getitem__(self, index):
        ...
        image_read.load()
        clip_image=self.processor(images=[image_read], return_tensors="pt")
        clip_image=clip_image['pixel_values']
        clip_image=clip_image.squeeze(0)

        image_origin = self.image_transforms(image_read)
        image_augment = image_origin

        #clip_image = image_origin
        if self.data_type == 1:
            image_augment = copy.deepcopy(image_read)
            image_augment = self.image_transforms(image_augment)
        return self.text_to_id[index], image_origin, int(self.label_list[index]), self.text_translation_to_id[self.data_id_list[index]], image_augment,self.clip_to_id[index],self.clip_attention_mask[index],clip_image
...
class Collate():
    ...
    def _collate(self, batch_data):
        ...
        clip_to_id=[torch.LongTensor(b[5]) for b in batch_data]
        clip_attention_mask=[torch.LongTensor(b[6]) for b in batch_data]
        clip_image=torch.FloatTensor(np.array([np.array(b[7]) for b in batch_data]))
        ...
        clip_attention_mask = run_utils.pad_sequence(clip_attention_mask, batch_first=True, padding_value=0)
        clip_to_id = run_utils.pad_sequence(clip_to_id, batch_first=True, padding_value=0)
        ...
        return text_to_id, torch.LongTensor(bert_attention_mask), image_origin, torch.LongTensor(text_image_mask), label, \
               text_translation_to_id, torch.LongTensor(tran_bert_attention_mask), image_augment, torch.LongTensor(tran_text_image_mask), target_labels,clip_to_id,torch.LongTensor(clip_attention_mask),clip_image
