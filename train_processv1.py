...
def train_process(opt, train_loader, dev_loader, test_loader, cl_model, critertion, log_summary_writer:SummaryWriter=None, tokenizer=None, image_id_list=None):
    ...
    # if opt.optim =='adam':
    #     optimizer = Adam(cl_model.parameters(), lr=opt.lr,betas=(opt.optim_b1, opt.optim_b2),weight_decay=1e-4)
    # elif opt.optim == 'adamw':
    #     optimizer = AdamW(cl_model.parameters(), lr=opt.lr,betas=(opt.optim_b1, opt.optim_b2),weight_decay=1e-4)
    # elif opt.optim == 'sgd':
    ...
    for epoch in trange(opt.epoch, desc='Epoch:'):
        ...
        for index, data in enumerate(train_loader_tqdm):
            ...
            with mcontext():
                with autocast(): 
                    origin_res, l_pos_neg, cl_lables, cl_self_loss,iamge_textsum,text_imagesum,loss_ita_hidden = cl_model(data_orgin=orgin_param, data_augment=augment_param, labels=labels, target_labels=target_labels,trainortest=1)
                    ...
                    loss = (classify_loss +0.2*total_text_image_loss_all+3*loss_ita_hidden) / opt.acc_batch_size
