...
import os
import argparse
import data_processv2 as data_process
# import train_process_debug as train_process
import train_processv2 as train_process
import torch
import torch.nn.modules as nn
import dev_processv2 as dev_process
import test_processv2 as test_process
...
# # import train_process_debug as train_process
# import train_process as train_process
# import torch
# import torch.nn.modules as nn
# import dev_process as dev_process
# import test_process as test_process
...
if __name__ == '__main__':
    ...
    parse.add_argument('-text_model', type=str, default='debertav3_base', help='language model')   #bert-base, bertweet-en
    ...
    parse.add_argument('-text_fuse_type', type=str, default='att', help='att, ave, max')
    parse.add_argument('-image_fuse_type', type=str, default='att', help='att, ave, max')
    ...
    parse.add_argument('-image_num_layers', type=int, default=6, help='The layer of image transformer')
    ...
    parse.add_argument("-queue_size", type=int, default=3584,help='single is 3584-32-1 multiple is 13568-32-1 hfm is 19968-32-acgrad4   for ddp,dp is another process')
    ...
    pretrained_dict = torch.load('./checkpoint/2023-05-31-11-43-01-HFM22-3-att-32-6-debertav3_base-/05-31-12-39-49-Acc-0.85862.pth',map_location='cpu')
    #print(pretrained_dict)
    cl_fuse_model_dict = cl_fuse_model.state_dict()
    # 关键在于下面这句，从model_dict中读取key、value时，用if筛选掉不需要的网络层 
    pretrained_dict = {key: value for key, value in pretrained_dict.items() if ( 'fuse_model.transformer_encoder'  in key)}
    #print(pretrained_dict)
    cl_fuse_model_dict.update(pretrained_dict)
    cl_fuse_model.load_state_dict({k.replace('module.', ''): v for k, v in                 
                    pretrained_dict.items()},strict=False)  
    ...
    if opt.cuda is True:
        ...
        if len(opt.gpu_num) > 1:
            if opt.gpu0_bsz > 0:
                ...
                cl_fuse_model.to(device)
                cl_fuse_model = torch.nn.parallel.DistributedDataParallel(cl_fuse_model, find_unused_parameters=True)
        else:
            cl_fuse_model = cl_fuse_model.cuda()
        critertion = critertion.cuda()
    #print(cl_fuse_model.fuse_model.transformer_encoder.layers[2].norm1)
    ...
    # elif opt.text_model == 'bertweet_en':
    #     tokenizer = AutoTokenizer.from_pretrained('./bertweet_en')
    # elif opt.text_model == 'debertav3_base':
    #     tokenizer = AutoTokenizer.from_pretrained('./debertav3_base_en')
    ...
    # elif opt.data_type == 'TumEmo':
    #     data_path_root = abl_path + 'dataset/data/TumEmo/'
    #     train_data_path = data_path_root + 'train.json'
    #     dev_data_path = data_path_root + 'dev.json'
    #     test_data_path = data_path_root + 'test.json'
    #     photo_path = data_path_root + '/dataset_image'
    #     image_coordinate = None
    #     data_translation_path = data_path_root + '/TumEmo.json'
    # else:
    ...
    #     train_process.train_process(opt,train_loader, dev_loader, test_loader, cl_fuse_model, critertion, log_summary_writer)  #使劲自动权重loss
    # elif opt.run_type == 2:
    #     print('\nTest Begin')
    #     model_path = "./checkpoint/2023-06-01-17-01-35-MVSA7-single-3-att-32-6-debertav3_base-/06-01-17-07-42-Acc-0.63053.pth"
    #     cl_fuse_model.load_state_dict({k.replace('module.', ''): v for k, v in                 
    #                    torch.load(model_path,map_location='cpu').items()},strict=False)
